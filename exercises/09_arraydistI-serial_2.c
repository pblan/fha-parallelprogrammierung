...
int main(int argc, char *argv[]) {
  ...
  enum { SEND_ARRAY = 1, SEND_PATTERN = 2 };
  ...
  if (procRank == 0) {
    ...
    for (int i = 1; i < procCount; ++i) {
      MPI_Send(vector, k, MPI_INT, i, SEND_ARRAY, MPI_COMM_WORLD);
    }

    for (int i = 1; i < procCount; ++i) {
      MPI_Send(&i, 1, MPI_INT, i, SEND_PATTERN, MPI_COMM_WORLD);
    }
  }
  ...
  if (procRank != 0) {
    // receiving array
    MPI_Status status;
    MPI_Probe(0, SEND_ARRAY, MPI_COMM_WORLD, &status);

    MPI_Get_count(&status, MPI_INT, &k);

    vector = (int *)malloc(k * sizeof(int));

    MPI_Recv(vector, k, MPI_INT, 0, SEND_ARRAY, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
